...
let data = new (function () {
	...
	input.match(new RegExp(this.productNumber + "[012].{14}", "g"))
	.forEach((jan) => {
		jan.trim()
	...
	this.sizes = input.match(/\d{1,2}号|(?<=\s)[LMS]+(?=\s)|フリー/g).filter((item, i, arr) => arr.indexOf(item) === i);
	this.colors = input.match(/(?<=ＩＳＴ価.+須.*)[一-龠ァ-ヴーぁ-ゔｱ-ｳﾞ々〆〤]+/g);
